...
function initGame() {
  ...
  renderInventory();
  // 注册返回按钮事件
  const backBtn = document.getElementById('scene-back-button');
...
function handleMinigame(hot) {
  // 检查是否需要先提供某个物品
  if (hot.requireItem) {
    if (!inventory.includes(hot.requireItem)) {
      showToast(hot.failMessage || '条件不满足。');
      return;
    }
    // 移除需求物品
    const idx = inventory.indexOf(hot.requireItem);
    if (idx >= 0) {
      inventory.splice(idx, 1);
      // 如果正在选中该物品则取消
      if (selectedItem === hot.requireItem) {
        selectedItem = null;
        firstSelected = null;
      }
      renderInventory();
    }
    showToast('你投入了一枚硬币，祭坛开始发光。');
  }
...
function handleZoom(hot) {
  ...
  zoom.items.forEach(zitem => {
    ...
    div.addEventListener('click', (e) => {
      ...
      e.stopPropagation();
      // 如果物品需要特定物品来清理或使用
      if (zitem.requireItem) {
        // 必须选择正确的物品才能继续
        if (selectedItem !== zitem.requireItem) {
          showToast(zitem.failMessage || '需要其他物品。');
          return;
        }
        // 移除使用的物品
        const idx = inventory.indexOf(selectedItem);
        if (idx >= 0 && zitem.removeUsed) {
          inventory.splice(idx, 1);
        }
        selectedItem = null;
        firstSelected = null;
        renderInventory();
        showToast('你用' + items[zitem.requireItem].name + '清理了尘土。');
        // 之后可以拾取该物品，继续执行
      }
      // 拾取物品
...
function renderInventory() {
  ...
  inventory.forEach((itemId, idx) => {
    ...
    if (selectedItem && itemId === selectedItem) {
      slot.classList.add('selected');
    }
    const img = document.createElement('img');
    img.src = items[itemId].img;
    img.alt = items[itemId].name;
    slot.appendChild(img);
    slot.addEventListener('click', () => {
      // 点击空槽位没效果
      const itemIdInner = inventory[idx];
      if (!itemIdInner) return;
      if (!selectedItem) {
        selectedItem = itemIdInner;
        firstSelected = itemIdInner;
        showToast(items[itemIdInner].name);
      } else if (selectedItem === itemIdInner) {
        selectedItem = null;
        firstSelected = null;
      } else {
        const secondItem = itemIdInner;
  ...
  // 如果背包为空，添加一个占位槽位以占据空间
  if (inventory.length === 0) {
    const placeholder = document.createElement('div');
    placeholder.className = 'inv-slot placeholder';
    inv.appendChild(placeholder);
  }
}
...
// 切换到指定场景
function goToScene(sceneId, fromBack = false) {
  if (!fromBack) {
    // 历史记录当前场景用于返回
